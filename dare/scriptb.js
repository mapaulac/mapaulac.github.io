...
function callFetch() {
    if (choseDare1){
        fetchDare = finalDare1;
    } else if (choseDare2){
        fetchDare = finalDare2;
    } else if (choseDare3){
        fetchDare = finalDare3;
    }
    fetch("https://gameofdare.herokuapp.com/bsend",
    {
        ...
        body: JSON.stringify({msg: `${fetchDare}`})
        //body: JSON.stringify({msg: "dare from website"}) // body data type must match "Content-Type" header
      })
        .then(res => {
            res.text()
                .then(txt => {
                    newdare = txt;
                    console.log(newdare) //dare sent from JSON
                });
        });
};
    ...
    fetch("https://gameofdare.herokuapp.com/bsend",
    {
        ...
        .then(res => {
            res.text()
                .then(txt => {
                    newFinalDare = txt;
                    console.log(newFinalDare) //dare sent from JSON
                    var newDareSpot = document.getElementById("part5NewDare");
                    newDareSpot.innerHTML = newFinalDare;
                });
        });
};
